		...
		if (textbox.mode = "box") {
			...
			if (view_enabled == true && textbox.position_set == false) {
				...
				if (view_enabled == true) {
					if (textbox.option2x != noone) {
						_option_x = textbox.option2x+camera_get_view_x(view_camera[view_current]);
						_option_y = textbox.option2y+camera_get_view_y(view_camera[view_current]);
					}else{
			...
			var _opt_h = string_height(opt2); // Text height
			var _oy = 0;
			
			if (textbox.option2y == noone) {
				...
				_oy = _option_y; //textbox.option2y;
				//_option_x = textbox.option2x;
			}
